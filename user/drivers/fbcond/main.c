/**
 * @file main.c
 * @brief fbcond — framebuffer 文本控制台 displaydev 驱动
 *
 * 通过 libdriver displaydev 框架注册为文本显示设备.
 * 将 DISPDEV_WRITE 中的文本渲染到 framebuffer.
 */
    ...
    for (int row = 0; row < h; row++) {
        ...
        if (st->bytes_per_pixel == 4) {
            ...
            for (int col = 0; col < w; col++) {
                line[col * 3] = b0; line[col * 3 + 1] = b1; line[col * 3 + 2] = b2;
            }
        }
    }
}
...
static void fbcon_putc(struct fbcon_state *st, uint32_t cp) {
    if (cp == '\n') { fbcon_newline(st); return; }
    if (cp == '\r') { st->cursor_x = 0; return; }
    if (cp == '\t') {
        int next = (st->cursor_x + 8) & ~7;
        if (next >= st->cols) fbcon_newline(st); else st->cursor_x = next;
        return;
    }
    if (cp == '\b') {
        if (st->cursor_x > 0) {
    ...
    if (st->cursor_x >= st->cols) fbcon_newline(st);
...
static int fbcon_write(struct display_device *dev, const void *buf, size_t len) {
    (void)dev;
    pthread_mutex_lock(&g_fbcon.lock);
    fbcon_write_bytes(&g_fbcon, (const uint8_t *)buf, len);
    pthread_mutex_unlock(&g_fbcon.lock);
    return (int)len;
}

static int fbcon_clear(struct display_device *dev) {
    (void)dev;
    pthread_mutex_lock(&g_fbcon.lock);
...
static int fbcon_scroll(struct display_device *dev, int lines) {
    (void)dev;
    pthread_mutex_lock(&g_fbcon.lock);
    fb_scroll_chars(&g_fbcon, lines);
    pthread_mutex_unlock(&g_fbcon.lock);
    return 0;
}

static int fbcon_set_cursor(struct display_device *dev, int row, int col) {
    (void)dev;
    pthread_mutex_lock(&g_fbcon.lock);
    g_fbcon.cursor_y = row;
    g_fbcon.cursor_x = col;
    pthread_mutex_unlock(&g_fbcon.lock);
    return 0;
}

static int fbcon_set_color(struct display_device *dev, uint8_t attr) {
    (void)dev;
    pthread_mutex_lock(&g_fbcon.lock);
...
static int fbcon_reset_color(struct display_device *dev) {
    (void)dev;
    pthread_mutex_lock(&g_fbcon.lock);
...
static struct display_ops fbcon_ops = {
    .write       = fbcon_write,
    .clear       = fbcon_clear,
    .scroll      = fbcon_scroll,
    .set_cursor  = fbcon_set_cursor,
    .set_color   = fbcon_set_color,
    .reset_color = fbcon_reset_color,
};
...
int main(void) {
    ...
    if (fb_handle == HANDLE_INVALID) {
        ulog_errf("[fbcond] fb_mem missing\n");
        return 1;
    }
    ...
    struct display_device dev = {
        ...
        .instance = 0,
        .ops      = &fbcon_ops,
        .caps     = DISPDEV_CAP_COLOR | DISPDEV_CAP_CURSOR | DISPDEV_CAP_SCROLL,
        .rows     = g_fbcon.rows,
        .cols     = g_fbcon.cols,
        .endpoint = env_get_handle("fbcon_ep"),
    };

    if (displaydev_register(&dev) < 0) {
        ulog_errf("[fbcond] register failed\n");
        return 1;
    }

    svc_notify_ready("fbcond");
    ulog_tagf(stdout, TERM_COLOR_LIGHT_GREEN, "[fbcond]", " ready\n");
