...
#include <xnix/udm/fb.h>

/* 继承的 capability handles */
...
int main(void) {
    ...
    if (sys_fb_info(&fb_info) < 0) {
        printf("[fbd] Failed to get framebuffer info\n");
        return 1;
    }

    printf("[fbd] FB: %ux%u@%u, pitch=%u\n", fb_info.width, fb_info.height, fb_info.bpp,
           fb_info.pitch);

    /* 映射 framebuffer 到用户空间 */
    fb_addr = sys_fb_map();
    if ((int)(uintptr_t)fb_addr < 0) {
        printf("[fbd] Failed to map framebuffer\n");
        return 1;
    }

    printf("[fbd] FB mapped at %p\n", fb_addr);
    fb_ready = 1;
