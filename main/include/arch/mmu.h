...
#define PAGE_ALIGN_UP(addr) \
    (((addr) + PAGE_SIZE - 1) & PAGE_MASK) /* 向上取整到页边界,例如PAGE_ALIGN_UP(4097) = 8192 */
#define PAGE_ALIGN_DOWN(addr) \
    ((addr) & PAGE_MASK) /* 向下取整到页边界,例如PAGE_ALIGN_DOWN(4097) = 4096 */
#define ADDR_TO_PFN(addr) \
    ((addr) >> PAGE_SHIFT) /* 地址转页帧号,例如ADDR_TO_PFN(0x12345000) = 0x12345 */
#define PFN_TO_ADDR(pfn) \
    ((pfn) << PAGE_SHIFT) /* 页帧号转地址,例如PFN_TO_ADDR(0x12345) = 0x12345000 */
